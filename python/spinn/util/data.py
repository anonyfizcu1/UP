...
def TrimDatasetByTokens(dataset, seq_length, eval_mode=False,
                sentence_pair_data=False, logger=None, allow_cropping=False):
    '''
    Avoid using excessively long training examples.
    Used for the processed nli dataset.
    '''
    assert allow_cropping == False
    ...
    if sentence_pair_data:
        trimmed_dataset = [
            example for example in dataset if len(
                example["premise_tokens"]) <= seq_length and len(
                example["hypothesis_tokens"]) <= seq_length]
    else:
        trimmed_dataset = [example for example in dataset if
                           len(example["tokens"]) <= seq_length]

    diff = len(dataset) - len(trimmed_dataset)
    if eval_mode:
        if diff > 0:
            # TODO: NOTE: only for nli
            logger.Log(
                "Warning: Discarding " +
                str(diff) +
                " over-length validation examples.")
            return trimmed_dataset
        else:
            assert allow_cropping or diff == 0, "allow_eval_cropping is false but there are over-length eval examples."
            if logger and diff > 0:
            ...
            for example in dataset:
                for i, token in enumerate(example[key]):
                    if token in vocabulary:
                        ...
                        unks += 1
                    tokens += 1
            print "Unk rate {:2.3f}%, downcase rate {:2.3f}%, upcase rate {:2.3f}%".format((unks * 100.0 / tokens), (lowers * 100.0 / tokens), (raises * 100.0 / tokens))
        else:
            for example in dataset:
...
def MakeTrainingIterator(
    ...
    def build_batches():
        ...
        for split in order_splits:
            ...
            keys = []
            for i in split:
                num_transitions = sources[3][i] # num_transitions equals num_tokens for gumbel tree
            ...
            for i, _ in keys:
                ...
                if len(batch) == batch_size:
                    ...
                    batch = []
            # if len(batch) >= 1: # new from SPINN. in previous code, insufficient batch cannot be trained
            #     batches.append(batch)
    ...
    def batch_iter():
        ...
        while True:
            ...
            if idx >= num_batches:
                # Start another epoch.
                batches = build_batches()   # TODO: less efficient here
                num_batches = len(batches)
                idx = 0
                order = range(num_batches)
                random.shuffle(order)
    ...
    def batch_iter_pickled_pack():
        '''
        build batch iter for picking and do infinite iter loop in main training loop
        '''
        batches = build_batches()
    ...
    train_iter = batch_iter_pickled_pack if smart_batches else data_iter
...
def MakeEvalIterator(
        ...
        bucket_eval=False):
    if bucket_eval:
        print 'XXXXXXX: in bucket ', len(sources), batch_size, limit
        return MakeBucketEvalIterator(sources, batch_size)[:limit]
    else:
        print 'XXXXXXX: not in bucket'
...
def MakeStandardEvalIterator(
    ...
    while True:
        ...
        batch_indices = order[start:start + batch_size]
        candidate_batch = tuple(source[batch_indices] if source is not None else None 
...
def MakeBucketEvalIterator(sources, batch_size):
    ...
    for i in range(num_batches):
        ...
        # batch = tuple(source[batch_indices] for source in sources if source is not None)
        batch = tuple(source[batch_indices] if source is not None else None for source in sources)    # for gumbel tree dist will be None
        batches.append(batch)
...
def PreprocessDataset(
    ...
    if tree_joint:
        ...
        # TODO: use TrimDataset previously
        dataset = TrimDatasetByTokens(
            dataset,
            seq_length,
            eval_mode=eval_mode,
    ...
    if sentence_pair_data:
        ...
        # print X.shape   # num_sample x maxlen (80) x 2 # premise, hypothesis
    ...
    maxstep = X.shape[1]
    if tree_joint:
        dist = np.transpose( np.array( [[example["premise_dist"] + [0.0] * (maxstep - len(example['premise_dist'])) for example in dataset],
                               [example["hypothesis_dist"] + [0.0] * (maxstep - len(example['hypothesis_dist'])) for example in dataset]],
                              dtype=np.float32 ), (1, 2, 0) )
            # padding with 0
        # silver_tree = np.transpose(np.array([[convert_brackets_to_string(get_brackets(example['premise_tree'])[0]) + \
                                            #   ['-1, -1'] * (maxstep - len(convert_brackets_to_string(get_brackets(example['premise_tree'])[0]))) \
                                            #   for example in dataset], \
                                            #  [convert_brackets_to_string(get_brackets(example['hypothesis_tree'])[0]) + \
                                            #   ['-1, -1'] * (maxstep - len(convert_brackets_to_string(get_brackets(example['hypothesis_tree'])[0]))) \
                                            #   for example in dataset]]), (1, 2, 0))
    silver_tree = np.array([[convert_brackets_to_string(get_brackets(example['premise_tree'])[0]) + \
                                            ['-1, -1'] * (maxstep - len(convert_brackets_to_string(get_brackets(example['premise_tree'])[0]))) \
                                            for example in dataset], \
                                            [convert_brackets_to_string(get_brackets(example['hypothesis_tree'])[0]) + \
                                            ['-1, -1'] * (maxstep - len(convert_brackets_to_string(get_brackets(example['hypothesis_tree'])[0]))) \
                                            for example in dataset]])
    silver_tree = np.transpose(silver_tree, (1,2,0))
        # print silver_tree.shape
        # print '|' * 20
        # batch x n x 2
    # print dist.shape
    # example = dataset[0]
    # print example['premise_tree']
    # print convert_brackets_to_string(get_brackets(example['premise_tree'])[0]) + \
                                            #   ['-1, -1'] * (maxstep - len(convert_brackets_to_string(get_brackets(example['premise_tree'])[0])))
    # print silver_tree.shape
    # print silver_tree
    # sys.exit(0)

    return X, transitions, y, num_transitions, example_ids, dist, silver_tree
...
def convert_brackets_to_string(brackets):
    brackets_str = []
    for x in brackets:
        brackets_str.append('{:d},{:d}'.format(x[0], x[1]))
    return brackets_str