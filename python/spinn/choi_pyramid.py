...
class ChoiPyramid(nn.Module):
    ...
    def forward(
        ...
        hh, _, masks, temperature = self.binary_tree_lstm(
            emb, example_lengths_var, dist=dist, temperature_multiplier=pyramid_temperature_multiplier)
    ...
    def get_samples(self, x, vocabulary, only_one=False):
        ...
        return token_sequences  # < 2 x batch_size >
...
class BinaryTreeLSTM(nn.Module):
    ...
    def __init__(self, word_dim, hidden_dim, intra_attention,
        ...
        self.SbSLoss = nn.NLLLoss(reduce=False)
    ...
    def select_composition(
        ...
        :param sytx_dist: <numSent, maxlen>
        :return:
        '''
        ...
        comp_weights = dot_nd(
            ...
            candidates=new_h)
        #sytx_dist = None # TODO: remove this line
        if self.training:
            if sytx_dist is None:
                ...
                # rb_bias = mask[:,1:]
                # rb_bias = torch.cat([rb_bias, torch.zeros_like(mask[:,0].unsqueeze(1))], dim=1)
                # rb_bias = mask - rb_bias
                # comp_weights = comp_weights + ALPHA * rb_bias.float()

                select_mask = st_gumbel_softmax(
                    logits=comp_weights, temperature=local_temperature,
                    mask=mask)

            else: # strong supervision:
                #print 'step-by-step supervision'
                
                ############
                # compute `select_mask`, essentially the min distance
                # choose the min value to combine

                # sytx_dist: <numSent x (curlen-1)>
                # mask: <numSent x (curlen-1)>

                y = sytx_dist - mask.float() * 200. # to choose the min dist
                # <numSent x (curlen-1)>

                ### different argmin schemes

                ## right-most argmin
                # y_len = y.size(1)
                
                # y_flip = np.flip(y.data.cpu().numpy(), axis=1)
                # y_flip_argmin = np.argmin(y_flip, axis=1)
                # y_argmin = Variable(torch.from_numpy(y_len - 1 - y_flip_argmin).cuda())

                # ## random argmin
                # y_argmin = (y - Variable(torch.rand(y.size()).cuda()) * 0.1 ).min(1)[1]

                ## pytorch built-in argmin (not steady)
                y_argmin = y.min(1)[1]
                # <numSent>

                select_mask = convert_to_one_hot(
                    indices=y_argmin,
                    num_classes=y.size(1)).float()
                # <numSent x (curlen-1)>

                ############
                # compute loss (XENT against one_hot)
                # cannot use CrossEntropyLoss because the number of categories may change

                p_action = masked_softmax(comp_weights, mask)
                logp_action = p_action.log()
                sbs_loss_not_reduced = self.SbSLoss(logp_action, y_argmin)
                # <numSent>

                # print 'mask', mask

                self.sbs_loss += (sbs_loss_not_reduced * mask[:,1].float()).sum()
                # xent is meaningful if there's at least three words
                # however, some sentences are shorter than others in a batch
                temperature_to_display = -1

                # compute acc:
                # print 'acc', (p_action.max(1)[1] == y_argmin).data
                self.sbs_acc +=  ((p_action.max(1)[1] == y_argmin).float() * mask[:,1].float()).sum()
            ...
            #print select_mask.data.numpy().T

        # select_mask: <batch x (curlen-1)>
        # e.g., [[0 0 1 0 0]]

        select_mask_expand = select_mask.unsqueeze(2)
        # <batch x (curlen-1) x 1>
        ...
        if self.training and sytx_dist is not None:
            old_dist_l = sytx_dist[:,:-1]
            old_dist_r = sytx_dist[:,1:]
            # <numSent x (curlen-2)>

            dist_mask_l = left_mask[:,:-1]
            dist_mask_r = right_mask[:,1:]
            # <numSent x (curlen-2)>

            new_dist = dist_mask_l * old_dist_l + dist_mask_r * old_dist_r
        else:
            new_dist = None
        return new_h, new_c, select_mask, selected_h, temperature_to_display, new_dist

    def forward(self, input, length, groundtruth_idx=None, temperature_multiplier=1.0, dist=None):
        '''

        :param input: <numSent x maxlen x dim> input embeddings
        :param length:
        :param groundtruth_idx:
        :param temperature_multiplier:
        :param dist: <numSent x maxlen> distance
        :return:
        '''
        if dist is not None:
            if dist.size(1) <= 1:
        ...
        self.n_total = F.relu((length_mask.float().sum(1)-2)).sum()
        ...
        for i in range(max_depth - 1):
            ...
            # NOTE: bug fix: max_depth = 2 no new_dist
            if max_depth > 2:
                dist = new_dist
            done_mask = length_mask[:, i + 1]   # whether comes to the end of the seq
